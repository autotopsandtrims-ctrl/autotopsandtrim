...
/* These are phone photos, mostly PORTRAIT (3000x4000), and the card box used to
   ...
   The fix is the BOX, not the fit. `contain` was tried and rejected: it shows the
   whole frame but letterboxes it, and the owner does not want borders anywhere.
   `cover` is exactly "zoom out as far as you can without a border appearing" —
   it scales the image to the smallest size that still fills the box. So keep
   cover and make the box SQUARE instead of a 250px landscape slot:

     old 430x200 slot, 3:4 photo, cover  ->  about 35% of the frame visible
     1:1 box,          3:4 photo, cover  ->  about 75% visible, still no borders

   object-position (emitted inline per card by card_shot()) therefore still does
   its job of landing each crop on the actual subject. */
...
/* Square + cover, same as the cards and the strip. contain was tried and pulled:
   it letterboxed every pair, and no borders is the rule. */
...
/* Square + cover. contain was tried here too and reverted — no borders anywhere
   on this site. A 1:1 cover crop takes a quarter off a portrait phone photo,
   which is the least a full-bleed tile can take. */
